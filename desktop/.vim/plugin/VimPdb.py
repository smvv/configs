...
class PdbIDE(bdb.Bdb):
    """Simulates a Python debugger in an IDE-like mode (unlike PDB, which acts
    as a command-line console debugger)."""
    ...
    MESSAGE_PROGRAM_ENDED_VIA_SYS_EXIT = \
            'Program ended via sys.exit(). Exit status: %d'
    MESSAGE_PROGRAM_ENDED_UNCAUGHT_EXCEPTION = \
            'Program ended due to an uncaught exception.'
    MESSAGE_NO_CONDITIONAL_BREAKPOINT = \
            'Error: No conditional breakpoint in current line'
    MESSAGE_BREAKPOINT_CONDITION = 'Breakpoint Condition: %s'
    MESSAGE_JUMP_ONLY_AT_BOTTOM_FRAME = \
            'Error: Can only jump to line within the bottom stack frame'
    MESSAGE_JUMP_ONLY_IN_CURRENT_FILE = \
            'Error: Can only jump to line within the currently debugged file'
    ...
    BREAKPOINT_TYPES = [BREAKPOINT_TYPE_REGULAR, BREAKPOINT_TYPE_CONDITIONAL, \
    ...
    def __init__(self):
        ...
        # Used in wait_in_debug method (method doesn't return until pause_debug
        # == False).
        ...
        # A queue of Bdb methods to run. This is used when VimPdb methods (as
        # opposed to Bdb methods) are called directly from the Vim file
        # (VimPdb.vim) - these methods (such as do_toggle_breakpoint) use this
        # queue to call Bdb methods (such as set_break) indirectly - it's done
        # this way so the Bdb methods will be called from this instance's
        # thread, and not from the Vim thread (which is the main thread). If the
        # Bdb methods were called directly, it would screw up Python
        # and Vim, and Vim will sometimes freeze\crash.  The run_queued_methods
        # method goes through this queue and executes the commands in it: each
        # item is a list of function name and parameters.
        self.methods_to_run = []
    ...
    def start_debugging(self, filename, stop_immediately = True, args = []):
        """Starts a debug session for a file. If stop_immediately is set,
        session is paused on the first line of program."""
        self.print_message(self.MESSAGE_STARTING_DEBUG)
        new_globals = { '__name__': '__main__' }
        new_locals = new_globals

        # So we won't break before we reach the first line of the script being
        # debugged.
        self.wait_for_script_start = True
        ...
        self.highlight_breakpoints(self.main_filename,
                *self.get_breakpoints_for_file(self.main_filename))

        # Replace main directory with running script's directory in front of
        # module search path.
        ...
        try:
            ...
            self.print_message(self.MESSAGE_PROGRAM_ENDED_VIA_SYS_EXIT %
                    (sys.exc_info()[1]))
            self.clear_current_line_highlighting()
            self.clear_breakpoints_highlighting()
        except:
    ...
    def do_toggle_breakpoint(self, filename, line_number, condition = None,
            temporary = False):
        ...
        # First, prepare a list of all available breakpoints for this file. Make
        # a copy so we won't be affected by changes.
        ...
        self.highlight_breakpoints(filename,
                *self.get_breakpoints_for_file(filename))
    ...
    def do_print_breakpoint_condition(self, filename, line_number):
        ...
        for breakpoint in breakpoint_instances:
            if (breakpoint.cond):
                self.print_message(self.MESSAGE_BREAKPOINT_CONDITION %
                        (breakpoint.cond))
                return


    def do_clear_all_breakpoints(self, filename = None):
        """Clears all breakpoints. If filename is specified, only breakpoints
        for that filename are cleared."""
    ...
    def do_clear(self, breakpoint_number):
        ...
        # Re-Highlight the breakpoints.
        self.highlight_breakpoints(self.current_filename,
                *self.get_breakpoints_for_file(self.current_filename))
    ...
    def do_eval(self, expression):
        ...
        try:
            value = eval(expression, self.current_frame.f_globals,
    ...
    def do_jump(self, filename, line_number):
        ...
        try:
            ...
            self.stack[self.current_stack_index] = \
                (self.stack[self.current_stack_index][0], line_number)
    ...
    def do_print_stack_trace(self):
        ...
        # Prepare the stack trace string. Skip the first two entries (which
        # aren't really part of the debugged code)
        for current_stack_frame in self.stack[2:]:
            (frame, line_number) = current_stack_frame

            if (frame is self.current_frame):
                output_stack_traces.append(self.current_stack_entry_prefix +
                        self.format_stack_entry(current_stack_frame))
            else:
                output_stack_traces.append(self.stack_entry_prefix +
                        self.format_stack_entry(current_stack_frame))
    ...
    def goto_current_line(self, frame, display = True):
        """Moves the cursor to the currently debugged line, in the appropriate
        file. If display == False, don't highlight or move the cursor."""
        if (not self.is_debugged()):
            return
        ...
        if (display):
            # Load the file for editing (even if the file is not currently
            # opened).
            self.open_file(filename)
            self.set_cursor_position(self.current_line, 0)
            self.highlight_current_line(self.current_filename,
                    self.current_line)
    ...
    def add_queued_method(self, function_name, *parameters):
        """Adds a method to the methods to run queue. It will be called
        indirectly by run_queued_methods"""
        self.methods_to_run.append([function_name, parameters])


    def run_queued_methods(self):
        """Executes any methods queued for execution. Used so that the methods
        will be executed from this instance's thread context (and not from the
        main Vim thread)."""
    ...
    def wait_in_debug(self, frame, traceback = None):
        ...
        (self.stack, self.current_stack_index) = self.get_stack(frame,
                traceback)
        self.current_frame = self.stack[self.current_stack_index][0]
    ...
    def is_breakpoint_enabled(self, filename, line):
        """Returns True if a breakpoint is enabled at the specified filename &
    ...
    def highlight_breakpoints_for_file(self, filename):
        ...
        self.highlight_breakpoints(self.canonic(filename),
                *self.get_breakpoints_for_file(self.canonic(filename)))
        ...
        for filename in file_breakpoints.keys():
            for line_number in file_breakpoints[filename]:
                for breakpoint in self.get_breaks(filename, line_number):
                    ...
                    if (breakpoint.cond):
                        new_breakpoint['type'] = \
                            self.BREAKPOINT_TYPE_CONDITIONAL
    ...
    def set_breakpoints(self, breakpoints):
        ...
        for breakpoint in breakpoints:
            ...
            # Set the breakpoint
            self.set_break(breakpoint['filename'], breakpoint['line'],
                    int(temporary), condition)

        # Re-highlight all of the breakpoints.
        self.highlight_breakpoints(self.get_active_filename(),
                *self.get_breakpoints_for_file(self.get_active_filename()))
    ...
    def load_breakpoints_from_file(self, filename):
        ...
        for line in breakpoints_file.xreadlines():
            ...
            if ((len(breakpoint_properties) < 3) \
                    or (len(breakpoint_properties) > 4)):
                self.print_message('Error: Invalid line #%d at file "%s"' \
                        % (index, filename))
                return

            (breakpoint_filename, breakpoint_line, breakpoint_type) = \
                breakpoint_properties[:3]
            breakpoint_type = breakpoint_type.lower()
            try:
                breakpoint_line = int(breakpoint_line)
            except ValueError:
                self.print_message('Error: Invalid breakpoint line number in' \
                        + 'line #%d at file "%s"' % (index, filename))
                return

            if (breakpoint_type not in self.BREAKPOINT_TYPES):
                self.print_message('Error: Invalid breakpoint type in' \
                        + ' line #%d at file "%s"' % (index, filename))
                return

            if ((breakpoint_type == self.BREAKPOINT_TYPE_CONDITIONAL) \
                    and (len(breakpoint_properties) != 4)):
                self.print_message('Error: Missing/invalid breakpoint' \
                        + ' condition in line #%d at file "%s"' \
                        % (index, filename))
                return
    ...
    def save_breakpoints_to_file(self, filename):
        ...
        for breakpoint in breakpoints:
            line = '%s\t%s\t%s' % (breakpoint['filename'], breakpoint['line'],
                    breakpoint['type'])
            if (breakpoint['type'] == self.BREAKPOINT_TYPE_CONDITIONAL):
    ...
    def get_conditional_breakpoints(self, filename):
        """Returns a list of line numbers with conditional breakpoints for a
        given filename."""
    ...
    def get_temporary_breakpoints(self, filename):
        """Returns a list of line numbers with temporary breakpoints for a given
        filename."""
    ...
    def get_breakpoints_for_file(self, filename):
        """Returns a tuple of (regular_breakpoints, conditional_breakpoints,
        temporary_breakpoints) for a given filename."""

        # Make a copy so we won't be affected by changes.
        regular_breakpoints = self.get_file_breaks(filename)[:]
        conditional_breakpoints = self.get_conditional_breakpoints(filename)
        temporary_breakpoints = self.get_temporary_breakpoints(filename)

        # Remove any breakpoints which appear in the regular_breakpoints list,
        # and are actually conditional or temporary breakpoints.
        for breakpoint in regular_breakpoints:
            if ((breakpoint in conditional_breakpoints) \
                    or (breakpoint in temporary_breakpoints)):
                regular_breakpoints.remove(breakpoint)

        return (regular_breakpoints, conditional_breakpoints,
                temporary_breakpoints)
    ...
    def format_stack_entry(self, stack_frame):
        ...
        stack_entry_string = self.stack_entry_format % ({'filename': filename,
            'dir': directory, 'line': line_number, 'function': function_name,
            'args': args, 'return_value': return_value, 'source_line':
            source_line})
    ...
    def user_line(self, frame):
        ...
        if (self.wait_for_script_start):
            if ((self.main_filename != self.canonic(frame.f_code.co_filename))
                    or (frame.f_lineno <= 0)):
                # Haven't reached the start of the script yet.
                return
    ...
    def user_exception(self, frame, (exc_type, exc_value, exc_traceback)):
        """
        Called if an exception occurs, but only if we are to stop at or just
        below this level.
        """
        if (self.quitting):
            return
    ...
    def highlight_breakpoints(self, filename, regular_breakpoints,
            conditional_breakpoints, temporary_breakpoints):
...
class VimPdb(PdbIDE):
    ...
    def highlight_breakpoints(self, filename, regular_breakpoints,
        ...
        self._set_lines_highlighting(conditional_breakpoints,
                self.CONDITIONAL_BREAKPOINT_GROUP)
        self._set_lines_highlighting(temporary_breakpoints,
                self.TEMPORARY_BREAKPOINT_GROUP)
    ...
    def highlight_current_line(self, filename, line):
        ...
        self.command(r'highlight link %s %s' % (self.CURRENT_LINE_GROUP,
            self.USER_DEFINED_CURRENT_LINE_GROUP))
    ...
    def run_method(self, function_name, *parameters):
        """
        Runs a method (using add_queued_method) and waits for its output;
        then prints it onto the screen.
        """
    ...
    def run_method_and_return_output(self, function_name, *parameters):
        """
        Runs a method (using add_queued_method) and waits for it to finish
        running; then returns its return value.
        """
    ...
    def _set_lines_highlighting(self, line_numbers, group_name):
        """Sets highlighting for a group of line numbers (given a group
        name)."""

        for line_number in line_numbers:
            self.command(r'syntax match %s "\%%%dl.\+"' % (group_name,
                line_number))

        # Old method - doesn't work for line #1, and when the previous line ends
        # with a quotation mark of the end of a string, for example.
